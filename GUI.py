...
def create_frame2(row_size,col_size,gm):
    ...
    while(1):
        ...
        if value >= 0:
            ...
            for i in range(gm.dim):
                for j in range(gm.dim):
                    if gm.solvr.current_state[i,j] > 0:
                        mine_x, mine_y = i,j
                        button_list[mine_x][mine_y].config(image=images['flag'])

            if x == 1:
                # for row in range(row_size):
                #     for col in range(col_size):
                #         if gm.board[row][col] == 1:
                #             button_list[row][col].config(image=images['mine'])
                messagebox.showinfo("solved", "You Won")
                root.destroy()
        else:
